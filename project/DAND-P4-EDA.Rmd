...
```
Superimposing a linear model confirms the conclusion from the previous plot.
...
ggplot(wine_quality, aes(x = citric.acid, y = log(fixed.acidity))) +
  ...
  stat_smooth(method = "lm")
```
There is a very slight linear normalization when plotting citric.acid and log(fixed.acidity).
